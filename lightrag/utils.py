...
def remove_think_tags(text: str) -> str:
    ...
    # Guard against non-string inputs from multimodal or unexpected payloads
    safe_text = "" if text is None else (text if isinstance(text, str) else str(text))
    return re.sub(
        r"^(<think>.*?</think>|.*</think>)", "", safe_text, flags=re.DOTALL
    ).strip()
